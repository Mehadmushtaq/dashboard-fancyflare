...
export default function ServiceCardsAddEdit({ isEdit }) {
  const icon = () => {
  ...
  const [newData, setNewData] = useState({
    ...
    is_main: prevData ? prevData.is_main : 1,
  });
  ...
  function isValidData() {
    ...
    else if (!newData.is_main) settypeErr(true);
    else return true;
    return false;
  }
  ...
  return (
    <>
      <div className="mainDashView">
        <div>
          <Header svg={icon} DashboardNavText={"Service Cards"} />
        </div>
              ...
              <div>
                ...
                <div className="ae-formFields">
                  ...
                  <div
                    ...
                    style={{ position: "relative", marginLeft: "25px" }}
                  >
                    <div className="ar-selectText">Placement</div>
                    <Select
                      options={isMainArr}
                      placeholder={"Select where to display the card"}
                      value={isMainArr[newData.is_main - 1]}
                      className="ae-select"
                      onChange={(data) => {
                        settypeErr(false);
                        handleChange({
                          target: { name: "is_main", value: data.value },
                        });
                      }}
                      styles={{
                        control: (baseStyles, state) => ({
